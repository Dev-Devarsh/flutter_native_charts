...
- (double)unprojectX:(double)xNdc {
  return chart_engine_unproject_x(_engine, xNdc);
}

- (double)unprojectY:(double)yNdc {
  return chart_engine_unproject_y(_engine, yNdc);
}

- (void)setTradeLineDraftActive:(BOOL)active
                             x1:(double)x1 y1:(double)y1
                             x2:(double)x2 y2:(double)y2 {
  chart_engine_set_trade_line_draft(_engine, active ? 1 : 0, x1, y1, x2, y2);
}

- (void)notifyTradeLineDrawEndWithX1:(double)x1 y1:(double)y1 x2:(double)x2 y2:(double)y2 {
  chart_engine_notify_trade_line_draw_end(_engine, x1, y1, x2, y2);
}

- (long long)tradeLineDrawCancelRevision {
  return chart_engine_trade_line_draw_cancel_revision(_engine);
}
