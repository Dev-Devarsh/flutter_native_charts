...
final class ChartHostView: UIView, UIGestureRecognizerDelegate {
  ...
  private var lastTradeLineDrawCancelRevision: Int64 = 0
  ...
  init(frame: CGRect,
    ...
    lastTradeLineDrawCancelRevision = engine.tradeLineDrawCancelRevision()

    let longPress = UILongPressGestureRecognizer(target: self, action: #selector(handleTradeLineLongPress(_:)))
  ...
  private func touchToData(_ p: CGPoint) -> (x: Double, y: Double) {
    let ndc = touchToNDC(p)
    return (engine.unprojectX(ndc.x), engine.unprojectY(ndc.y))
  }

  func resetTradeLineDrawing() {
    engine.setTradeLineDraftActive(false, x1: 0, y1: 0, x2: 0, y2: 0)
    isDrawingTradeLine = false
  }

  private func pollTradeLineDrawCancel() {
    let revision = engine.tradeLineDrawCancelRevision()
  ...
  private func cancelTradeLineDraw() {
    resetTradeLineDrawing()
  }

  @objc private func handleTradeLineLongPress(_ recognizer: UILongPressGestureRecognizer) {
    pollTradeLineDrawCancel()
    let loc = recognizer.location(in: self)
    let plot = plotRectForGestures()
    guard plot.contains(loc) else {
    ...
    case .began:
      ...
      drawStartX = data.x
      drawStartY = data.y
      engine.setTradeLineDraftActive(true, x1: data.x, y1: data.y, x2: data.x, y2: data.y)
    case .changed:
      guard isDrawingTradeLine else { return }
      pollTradeLineDrawCancel()
      guard isDrawingTradeLine else { return }
      engine.setTradeLineDraftActive(true, x1: drawStartX, y1: drawStartY, x2: data.x, y2: data.y)
    case .ended:
      guard isDrawingTradeLine else { return }
      pollTradeLineDrawCancel()
      guard isDrawingTradeLine else { return }
      isDrawingTradeLine = false
      engine.notifyTradeLineDrawEndWithX1(drawStartX, y1: drawStartY, x2: data.x, y2: data.y)
