...
// After viewport cull, at most this many candles become GPU vertices. Beyond
...
CHART_ENGINE_EXPORT int chart_engine_get_x_ticks(void* engine, double* out, int max_count) {
  ...
  const int total = static_cast<int>(state->x_ticks.size());
  if (out != nullptr && max_count > 0) {
    const int n = std::min(max_count, total);
...
CHART_ENGINE_EXPORT int chart_engine_get_y_ticks(void* engine, double* out, int max_count) {
  ...
  std::lock_guard<std::mutex> lock(state->mtx);
  state->ensureTicksFresh();
  const int total = static_cast<int>(state->y_ticks.size());
  if (out != nullptr && max_count > 0) {
    const int n = std::min(max_count, total);
